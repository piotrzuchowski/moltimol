...
def generate_psi4geom_files(
    ...
    Rs = sample_R_beta_mode(n_samples, r_min, r_max, mode_frac=mode_frac, concentration=concentration)
...
def run_propsapt_batch(
    ...
    if out_npz is None:
        out_npz = f"propsapt_batch_{batch_index:03d}.npz"
...
def sample_dimer_geometries(
    ...
    for i, R in enumerate(Rs):
        ...
        _, symbols, coords, mol_string = molmol.merge_monomers_jacobi_XYZ(
            fileA,
            fileB,
            R=R,
            theta=theta,
    ...
    if out_npz:
        np.savez_compressed(out_npz, **{c: csv_df[c].to_numpy() for c in csv_df.columns})
    return data, csv_df
